...
import { MobileClientSearch as MobileClientSearchComponent } from "@/components/mobile-views/mobile-client-search"
import { MobileAppointmentDialog } from "@/components/mobile-views/mobile-appointment-dialog"
import { MobileClientDetails } from "@/components/mobile-views/mobile-client-details"
import { MobileDatePickerSheet } from "@/components/mobile-date-picker-sheet"
  ...
  const serviceRooms: ServiceRoom[] =
    activeClinic?.config?.cabins
      ...
      .map((cabin) => ({
        ...
        name: cabin.name,
        // Fix the color handling to ensure it's properly formatted for Tailwind
        color: cabin.color.startsWith("#")
          ? `bg-[${cabin.color}]`
          : cabin.color.startsWith("bg-")
            ? cabin.color
            : `bg-${cabin.color}`,
      ...
      <div className="min-w-fit">
          ...
          >
            ...
            {serviceRooms.map((room) => (
              <div
                key={room.id}
                className={cn(
                  room.color.startsWith("#") ? "" : room.color,
                  "text-white text-xs p-2 text-center font-medium border-r last:border-r-0",
                )}
                style={room.color.startsWith("#") ? { backgroundColor: room.color } : {}}
              >
                {room.abbrev}
              </div>
            ))}
        ...
        <div className="relative">
          {timeSlots.map((time) => (
            ...
            >
              <div className="p-2 text-sm text-purple-600 border-r bg-white" style={{ position: "sticky", left: 0 }}>
                {time}
              </div>
              {serviceRooms.map((room) => (
                <div
          ...
          <CurrentTimeIndicator currentTime={currentTime} isMobile={true} />
        </div>
      </div>
    </div>
  )
